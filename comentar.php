...
<body>
    <!-- Виведення шапки сайту -->
     <p class='product_text' align="center" ><img  class = "product_img" src='img/png-transparent-computer-icons-magazine-windows-95-icon-logo-sign-signage.png' >Коментарі для "<?php echo $username; ?>"</p>
...
<div align="center" >
<?php
    $comments = mysqli_query($connect, "SELECT * FROM `comments` WHERE `comments`.`user` = '$username'");
    $comments = mysqli_fetch_all($comments);
                foreach ($comments as $comments) { ?>
                        <div class="vis"><td><h2>Дата <?= $comments[1] ?> Статус "<?= $comments[4] ?>"</h2></td>
                        <td><h3>Коментар<br><?= $comments[3] ?></h3></td></div><br>
                        <?php if($comments[4] == 'Новий') { ?>
                <div align="center"><form action="vendor/new_comments_ok.php" method="post">
                <input type="hidden" name="id" value="<?= $comments[0] ?>">
                <button class="btn-buy" type="submit">Прочитаний</button>
            </form><br></div>
    <?php }} ?>
    </div>
