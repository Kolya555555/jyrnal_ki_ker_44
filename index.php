...
.product_text{
    ...
    font-family: 'cursive;';
     color: black;}
.vis{
 background: linear-gradient(to bottom, #cc99ff 0%, #66ffcc 104%);
    border-radius: 10px;
...
<script language="JavaScript">  alert('Авторизація пройшла успішно!');
</script>
...
<div class="vis" align="center">
    <h1>Вітаємо у робочому кабінеті "<?php echo $username; ?>"</h1>
    <h2>Сьогодні: <?php echo $date; ?></h2>

<table>
    <tr>
        <td><img  class = "logo_img" src='img/maxresdefault.jpg'></td>
        <td><img  class = "logo_img" src='img/post.png'></td>
    </tr>
</table>
    <?php
    $comments = mysqli_query($connect, "SELECT * FROM `comments` WHERE `comments`.`user` = '$username' and `comments`.`statys` = 'Новий'");
    $comments = mysqli_fetch_all($comments);
     if($comments != null){
            foreach ($comments as $comments) { ?>
                <h2><a style="color: red;" href="comentar.php"> !!Наявні нові коментарі !!</a></h2>
        <?php }} ?>
    ...
    $request = mysqli_fetch_all($request);
     ...
     <td><?php $nomer=$request[7] ?></td> <?php }

    $rozdil = mysqli_query($connect, "SELECT * FROM `7rozdil` WHERE  `7rozdil`.`vidmitca` = '' and `7rozdil`.`namer_grupu` = '$nomer'");
    $rozdil = mysqli_fetch_all($rozdil);

     if($rozdil != null){
            foreach ($rozdil as $rozdil) { ?>
                <h2><a style="color: red;" href="7.php"> !!Невиконаны плани !!<br></a>
                   <td>Дата <?= $rozdil[1] ?></td><br>
                   <td>Опис: <?= $rozdil[2] ?></td></h2>
        <?php }} ?></div>
